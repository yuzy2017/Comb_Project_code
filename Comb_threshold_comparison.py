...
overlap = st.number_input(r"$\text{Overlap}$:", value=0.9)
...
if st.button('Calculate'):
    ...
    # Calculate 'g'
    g = (hbar * omega_s * omega_i * omega_p * chi2 ** 2 * overlap ** 2) / (
                16 * epsilon_0 * L_res * neff ** 6) * Fi ** 2 * Fs ** 2 * Fp ** 4 * (neff ** 2 - 1) ** 2 * 1e7
    # g = (2*pi*1e6)**2
...
""")
st.latex(r"""
Efficiency SHG= 0.1\ set\ constant
""")
st.latex(r"""
...
- $\text{overlap}$: Overlap integral of the modes  
